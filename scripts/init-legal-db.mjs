...
async function initLegalDb() {
  console.log("Connecting to Neon PostgreSQL to initialize legal_documents table...");
  ...
  `;
  console.log("✓ Table 'legal_documents' verified/created.");

  // Insert or update Privacy Policy
  await sql`
    INSERT INTO legal_documents (id, title, last_updated, summary, sections, updated_at)
    VALUES (
      ${defaultPrivacyPolicy.id},
      ${defaultPrivacyPolicy.title},
      ${defaultPrivacyPolicy.lastUpdated},
      ${defaultPrivacyPolicy.summary},
      ${JSON.stringify(defaultPrivacyPolicy.sections)}::jsonb,
      CURRENT_TIMESTAMP
    )
  ...
  `;
  console.log("✓ Privacy Policy seeded to Neon.");

  // Insert or update Terms & Conditions
  await sql`
    INSERT INTO legal_documents (id, title, last_updated, summary, sections, updated_at)
    VALUES (
      ${defaultTermsConditions.id},
      ${defaultTermsConditions.title},
      ${defaultTermsConditions.lastUpdated},
      ${defaultTermsConditions.summary},
      ${JSON.stringify(defaultTermsConditions.sections)}::jsonb,
      CURRENT_TIMESTAMP
    )
    ON CONFLICT (id) DO UPDATE SET
      title = EXCLUDED.title,
      ...
      updated_at = CURRENT_TIMESTAMP;
  `;
  console.log("✓ Terms & Conditions seeded to Neon.");

  const rows = await sql`SELECT id, title, last_updated FROM legal_documents;`;
  console.table(rows);
}

initLegalDb().catch((err) => {
  console.error("Error initializing legal documents in Neon:", err);
  process.exit(1);
});
